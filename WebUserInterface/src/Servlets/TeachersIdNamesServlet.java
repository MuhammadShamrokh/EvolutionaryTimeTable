...
//url: LocalHost:8080/TimeTable/pages/algopage/teachersNamesList
public class TeachersIdNamesServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest i_Request, HttpServletResponse i_Response)
        ...
        if(!user.getIsAlgoActivated(Integer.parseInt(managerIndex)))
        {
            ...
            try (PrintWriter out = i_Response.getWriter()) {
                Gson gson = new Gson();
                String json = gson.toJson(user.getTeachersIdNamesList(Integer.parseInt(managerIndex)));
                out.println(json);
                out.flush();
            }
