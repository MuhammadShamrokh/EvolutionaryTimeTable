...
public class BestSolutionByTeacherIDServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest i_Request, HttpServletResponse i_Response)
        ...
        String teacherID=i_Request.getParameter(Constants.TEACHER_ID);
        ...
        if(!user.getIsAlgoActivated(Integer.parseInt(managerIndex)))
        {
            ...
            try (PrintWriter out = i_Response.getWriter()) {
                Gson gson = new Gson();
                String json = gson.toJson(user.getBestSolutionByTeacherID(Integer.parseInt(managerIndex), Integer.parseInt(teacherID)));
                out.println(json);
                out.flush();
            }
